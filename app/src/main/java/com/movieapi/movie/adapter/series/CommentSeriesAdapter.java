...
public class CommentSeriesAdapter extends RecyclerView.Adapter<CommentSeriesAdapter.SeriesHolder> {
    ...
    public CommentSeriesAdapter(Context context, List<CommentModel> commentModelList, CommentItemListener listener) {
    ...
    @NonNull
    @Override
    public CommentSeriesAdapter.SeriesHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new SeriesHolder(LayoutInflater.from(context).inflate(R.layout.item_comments, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull CommentSeriesAdapter.SeriesHolder holder, int position) {
                    ...
                    public void onClick(View v) {
                        if(isLiked){
                            if (commentModel.getTotalLikeComment() == 0){
                                commentController.insertTotalLikeCmt(seriesId, idCmt, commentModel.getTotalLikeComment());
                            }else {
                                commentController.insertTotalLikeCmt(seriesId, idCmt, commentModel.getTotalLikeComment() - 1);
                            }
                        }else
                            commentController.insertTotalLikeCmt(seriesId, idCmt, commentModel.getTotalLikeComment() + 1);
